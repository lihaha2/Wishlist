...
export const Home = ()=>{
  const ChangeSize = ()=>{
    if(localStorage.getItem('reduce') === 'true'){
      ...
      localStorage.setItem('reduce', 'false')
    }else{
      ...
      localStorage.setItem('reduce', 'true')
    }
  ...
  const newListHandler = ()=>{
    ...
    if (title === '') {
      window.alert('Заполните поле')
    }else{
      document.querySelectorAll('.card__link > h3').forEach(e => e.innerHTML === title ? i = i + 1 : false)
      document.querySelector('#newList').value = ''
      i > 0 ? window.alert('Такой список уже существует!') : setToUserWishlist(title, localStorage.getItem('unique'))
    }
  ...
  const newWishHandler = ()=>{
    ...
    title && text === '' ? window.alert('Заполните поля') : setToUserWishes(title, localStorage.getItem('unique'), text)
    document.querySelector('#wishText').value = ''
    document.querySelectorAll('details')[1].open = false
  }
  const deleteListHandler = (title)=>{
    let result = window.confirm(`Вы точно хотите УДАЛИТЬ список: 
    ${title}`)
    result ? deleteUserWishlist(title,localStorage.getItem('unique')) : console.log('')
  }
  const Wishlists = ()=>{
  ...
  const signOutHandler = ()=>{
    let result = window.confirm('Вы точно хотите выйти из учётной записи?')
    result ? signOutUser() : console.log('')
  } 
    ...
    <div className="home">
      <header className="home__head">
        ...
        <div className="home__header-change">
          <div className="change__img" title="Показать/Скрыть навигацию" onClick={ChangeSize}>
